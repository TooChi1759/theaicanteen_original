...
export function TipCard({ tip }: { tip: Tip }) {
  const Icon = tip.icon;
  return (
    <article className="card-surface grain group flex h-full flex-col p-6 transition-all duration-500 ease-out-expo hover:-translate-y-1 hover:border-amber/25 hover:shadow-lift">
      ...
      <h3 className="font-serif text-lg font-semibold text-ivory">{tip.title}</h3>
      <p className="mt-2 flex-1 text-sm leading-relaxed text-stone-light">{tip.description}</p>
      <span className="mt-5 inline-flex items-center gap-1.5 text-xs text-stone">
